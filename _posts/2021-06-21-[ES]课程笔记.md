...
1. config目录下elasticsearch.yml中修改集群名称
...
1. 快速检查集群健康状况
	...
	| color  | description                                                  |
	| ------ | ------------------------------------------------------------ |
	| Green  | 所有primary shard和replica shard都是active状态               |
	| Yellow | 所有primary shard处于active状态<br />部分replica shard处于不可用状态（如果只有一台机器，statue一定是黄色，因为没办法备份） |
	| Red    | 不是所有index的primary shard都处于active状态，部分所有数据丢失 |
...
### Example2:对特定名称计算tags下分组（先搜索再分组）
...
### Example3:先分组再算平均值，在每个tags计算商品平均价格
...
### Example5:先按价格区间分类，然后按tags分类，最后aggs
...
- 复杂的分布式机制隐藏：分片，副本，负载均衡，请求路由，集群扩容，shrad重分配
- 垂直扩容与水平扩容
